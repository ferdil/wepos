...
export default {
    ...
    getters: {
        getCartLineItems(state) {
            return state.cartdata.line_items;
        },
        ...
        getTotalTax(state) {
            ...
            if (
                state.settings.woo_tax != undefined &&
                state.settings.woo_tax.wc_tax_display_cart == "incl"
            ) {
                taxLineTotal = 0;
            }
    ...
    mutations: {
        ...
        addToCartItem(state, product) {
            var cartObject = {};
            cartObject.product_id =
                product.parent_id === 0 ? product.id : product.parent_id;
            ...
            var index = state.cartdata.line_items.findIndex(function (el) {
                return (
                    el.product_id == cartObject.product_id &&
                    el.variation_id == cartObject.variation_id
                );
            });
            ...
            if (index < 0) {
                ...
                if (
                    Helper.hasStock(product, state.cartdata.line_items[index].quantity)
                ) {
        ...
        toggleEditQuantity(state, itemKey) {
            state.cartdata.line_items[itemKey].editQuantity = !state.cartdata
                .line_items[itemKey].editQuantity;
        },
                ...
                weLo_.forEach(state.cartdata.fee_lines, (item, key) => {
                    if (item.type == "discount") {
                        if (item.discount_type == "percent") {
                            state.cartdata.fee_lines[key].total =
                                "-" + (payload.getSubtotal * Math.abs(item.value)) / 100;
                        } else {
                ...
                weLo_.forEach(state.cartdata.fee_lines, (item, key) => {
                    if (item.type == "fee") {
                        if (item.fee_type == "percent") {
                            state.cartdata.fee_lines[key].total = (
                                (payload.getSubtotal * Math.abs(item.value)) /
                                100
                            ).toString();
                        } else {
                            state.cartdata.fee_lines[key].total = Math.abs(
                                item.value
                            ).toString();
                        }
                    }
